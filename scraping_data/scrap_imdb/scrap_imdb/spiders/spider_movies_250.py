...
class ImdbSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request(url='https://www.allocine.fr/boxoffice/france/sem-2023-07-05/', headers={
    ...
    def parse(self, response):
        ...
        links = response.css('div.meta h2.meta-title a.meta-title-link::attr(href)').getall()
        for link in links:
    ...
    def parse_movie(self, response):
        items = ScrapImdbItem()
        titre_original = response.xpath('(//div[@class="titlebar titlebar-page"]//div[@class="titlebar-title titlebar-title-lg"]//text())').get()
        ...
        # desciption = response.xpath('(//p[@class="sc-5f699a2-3 lopbTB"]//span[@class="sc-5f699a2-2 cxqNYC"])//text()').get().strip('()')
