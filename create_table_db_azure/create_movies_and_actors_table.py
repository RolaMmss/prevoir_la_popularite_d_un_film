import csv
import pyodbc
from create_table_script import create_table_and_insert_data, delete_table, connect_to_database
...
with open(csv_file_path, 'r', encoding='utf-8') as csvfile:
    ...
    for i, row in enumerate(csvreader, start=1):
        try:
            query = '''
            INSERT INTO movies (titre, date, duree, genre, realisateur, distributeur, nationalites, langue_d_origine, type_film, annee_production, nombre_article, description, film_id_allocine, image)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
            '''
            cursor.execute(query, (
                ...
                row['annee_production'], row['nombre_article'], row['description'], row['film_id_allocine'], row['image']
            ))
            conn.commit()
